...
#define ODBC_GATEWAY_DEFAULT_ADDR    "127.0.0.1"
#define ODBC_GATEWAY_DEFAULT_PORT    8066
...
void SetDsnDefaultFild(MADB_Dsn *Dsn) {
    Dsn->IsTcpIp = TRUE;

    if (Dsn->ServerName == NULL)
    {
        Dsn->ServerName = _strdup(ODBC_GATEWAY_DEFAULT_ADDR);
    }

    if (Dsn->Port == 0)
    {
        Dsn->Port = ODBC_GATEWAY_DEFAULT_PORT;
    }
...
void DSN_Set_Schema(SQLHANDLE Connection)
{
  MADB_Stmt *Stmt= NULL;
  SQLRETURN ret= SQL_ERROR;
  char Schema[65];
  MADB_Dsn *Dsn= (MADB_Dsn *)GetWindowLongPtr(GetParent(hwndTab[0]), DWLP_USER);
  HWND DbCombobox= GetDlgItem(hwndTab[1], cbSchema);
  ...
  if (SQLExecDirect((SQLHSTMT)Stmt, (SQLCHAR *)"SHOW SCHEMAS", SQL_NTS) != SQL_SUCCESS)
    goto end;

  SQLBindCol(Stmt, 1, SQL_C_CHAR, Schema, 65, 0);
  ComboBox_ResetContent(DbCombobox);
  while (SQLFetch(Stmt) == SQL_SUCCESS)
  {
    ComboBox_InsertString(DbCombobox, -1, Schema);
  }
  if (Dsn->Schema)
  {
    int Idx= ComboBox_FindString(DbCombobox, 0, Dsn->Schema);
    ComboBox_SetCurSel(DbCombobox, Idx);
  }
  ComboBox_SetMinVisible(GetDlgItem(hwndTab[1], cbSchema), 5);
...
void DSN_Set_Database(SQLHANDLE Connection)
{
  ...
  if (SQLAllocHandle(SQL_HANDLE_STMT, Connection, (SQLHANDLE *)&Stmt) != SQL_SUCCESS)
	...
	if (SQLExecDirect((SQLHSTMT)Stmt, (SQLCHAR *)"SHOW CATALOGS", SQL_NTS) != SQL_SUCCESS)
		goto end;
...
INT_PTR CALLBACK DialogDSNProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND:
    ...
    case pbFpListBrowse:
      ...
      OpenCurSelection= OpenCurSelection && !res;
      return res;
    case pbConnectBrowse:
        res=   SelectPath(hDlg, txtConnectCfg, L"Select Connecting config File", FALSE, TRUE);
        return res;
    case rbTCP:
	  case rbPipe:
