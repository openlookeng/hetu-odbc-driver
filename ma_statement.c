                                    ...
                                    SQLSMALLINT NameLength3, char *ColumnName, SQLSMALLINT NameLength4)
{
  ...
  p= StmtStr;
  length = _snprintf_s(StmtStr, sizeof(StmtStr), sizeof(StmtStr) - 1, "SELECT * FROM (SELECT CAST('NULL' AS VARCHAR(2048)) AS TABLE_CAT, "
                                 "CAST('NULL' AS VARCHAR(2048)) AS TABLE_SCHEM, CAST('NOT NULL' AS VARCHAR(2048)) AS TABLE_NAME, "
                                 "CAST('NOT NULL' AS VARCHAR(2048)) AS COLUMN_NAME, CAST('NOT NULL' AS VARCHAR(2048)) AS GRANTOR, "
                                 "CAST('NOT NULL' AS VARCHAR(2048)) AS GRANTEE, CAST('NOT NULL' AS VARCHAR(2048)) AS PRIVILEGE, "
                                 "CAST('NULL' AS VARCHAR(2048)) AS IS_GRANTABLE) WHERE 1=0 ");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
   length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, "ORDER BY TABLE_CAT, TABLE_SCHEM, TABLE_NAME, COLUMN_NAME, PRIVILEGE");
   p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
                                    ...
                                    char *TableName, SQLSMALLINT NameLength3)
{
  ...
  length = _snprintf_s(StmtStr, sizeof(StmtStr), sizeof(StmtStr) - 1, "SELECT * FROM (SELECT CAST('NULL' AS VARCHAR(2048)) AS TABLE_CAT, "
                                  "CAST('NULL' AS VARCHAR(2048)) AS TABLE_SCHEM, CAST('NOT NULL' AS VARCHAR(2048)) AS TABLE_NAME, "
                                  "CAST('NULL' AS VARCHAR(2048)) AS GRANTOR, CAST('NOT NULL' AS VARCHAR(2048)) AS GRANTEE, "
                                  "CAST('NOT NULL' AS VARCHAR(2048)) AS PRIVILEGE, CAST('NULL' AS VARCHAR(2048)) AS IS_GRANTABLE) WHERE 1=0 ");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);

  length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, "ORDER BY TABLE_CAT, TABLE_SCHEM, TABLE_NAME, PRIVILEGE");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
...
/* {{{ ONEQ_AllCatalogs */
SQLRETURN ONEQ_AllCatalogs(MADB_Stmt *Stmt, char *CatalogNames, int CatNamesLen)
{
    ...
    retcode = SQLExecDirect(hstmt,
        (SQLCHAR*) "SELECT CATALOG_NAME FROM SYSTEM.METADATA.CATALOGS"
        , SQL_NTS);
    if ( SQL_SUCCESS != retcode )
        return retcode;
    ...
    for (int i = 0; ; i++) {
        ...
        if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
            ...
            if (strcat_s(CatalogNames, CatNamesLen, ";") != 0) {
                return SQL_ERROR;
            }
        }
        else {
    ...
    //Free the allocated Statement Handler
    ...
    SQLSMALLINT TableNameLength, char *TableType, SQLSMALLINT TableTypeLength)
{
    ...
    */
    ...
    //DB server does not support either of these quote, only ' is valid
    if ((strlen("'") >= sizeof(Quote)) || (strcpy_s(Quote, sizeof(Quote), "'") != 0)) {
        return SQL_ERROR;
    }

    //Escape Character, should be backslash in ODBC
    if ((strlen("\\") >= sizeof(Escapechar)) || (strcpy_s(Escapechar, sizeof(Escapechar), "\\") != 0)) {
        return SQL_ERROR;
    }
    ...
    */
    ...
    {
        MADB_InitDynamicString(&StmtStr, "SELECT CATALOG_NAME AS TABLE_CAT, CAST(NULL AS CHAR(64)) AS TABLE_SCHEM, "
            "CAST(NULL AS CHAR(64)) AS TABLE_NAME, NULL AS TABLE_TYPE, NULL AS REMARKS "
    ...
    */
    else if (CatalogName != NULL && CatalogNameLength == 0 && TableName != NULL && !TableNameLength &&
        SchemaName && SchemaNameLength && !strcmp(SchemaName, SQL_ALL_SCHEMAS))
    {
        ...
        MADB_InitDynamicString(&StmtStr, "SELECT NULL AS TABLE_CAT, schema_name AS TABLE_SCHEM, "
                    "CAST(NULL AS CHAR(64)) AS TABLE_NAME, CAST(NULL AS CHAR(64)) AS TABLE_TYPE, "
                    "NULL AS REMARKS FROM ("
                    "SELECT * FROM \"",
                    524288, 512); 
                MADB_DynstrAppend(&StmtStr, Tabptr);
                MADB_DynstrAppend(&StmtStr, "\".INFORMATION_SCHEMA.SCHEMATA ");

                while ((Tabptr = strtok_s(NULL, ";", &Tabtemp)) != NULL)
                {
                    MADB_DynstrAppend(&StmtStr, "UNION ALL ");
                    MADB_DynstrAppend(&StmtStr, "SELECT * FROM \"");
                    MADB_DynstrAppend(&StmtStr, Tabptr);
                    MADB_DynstrAppend(&StmtStr, "\".INFORMATION_SCHEMA.SCHEMATA ");
                }
    ...
    */
    ...
    {
        MADB_InitDynamicString(&StmtStr, "SELECT NULL AS TABLE_CAT, CAST(NULL AS CHAR(64)) AS TABLE_SCHEM, "
            "CAST(NULL AS CHAR(64)) AS TABLE_NAME, 'TABLE' AS TABLE_TYPE, NULL AS REMARKS "
            "UNION "
            "SELECT NULL, NULL, NULL, 'VIEW', NULL "
            "UNION "
            "SELECT NULL, NULL, NULL, 'SYSTEM TABLE', NULL",
            8192, 512);
        goto RETEXEC;

    }

    //no DSN CatalogName and no input return an empty resultset. REAL EMPTYSET NOT RESULTSET WITH NULL!
    else if (!CatalogName && !(Stmt->Connection->Dsn->Catalog))
    {
        //Using this create a null result, or it will return an error.
        MADB_InitDynamicString(&StmtStr, "SELECT * FROM (SELECT NULL AS TABLE_CAT, CAST(NULL AS CHAR(64)) AS TABLE_SCHEM, "
            "CAST(NULL AS CHAR(64)) AS TABLE_NAME, 'TABLE' AS TABLE_TYPE, NULL AS REMARKS) WHERE 1=0"
            , 8192, 512);
        goto RETEXEC;
    }
    else
    {
        if (CatalogName)
        {
            ...
            if (CatalogNameIsEmptyStr)
            {
                //inside else, using this to generate a null resultset, or ORDER BY will return error.
                MADB_InitDynamicString(&StmtStr, "SELECT table_catalog AS TABLE_CAT, table_schema AS TABLE_SCHEM, "
                    "table_name AS TABLE_NAME, table_type AS TABLE_TYPE, NULL AS REMARKS FROM SYSTEM.INFORMATION_SCHEMA.TABLES WHERE 1=0"
                    , 8192, 512);
                goto RETEXEC;
            }
            else
            {
                ...
                MADB_InitDynamicString(&StmtStr, "SELECT * FROM (SELECT TABLE_CATALOG AS TABLE_CAT, TABLE_SCHEMA AS TABLE_SCHEM, TABLE_NAME, "
                    "CASE TABLE_SCHEMA WHEN 'information_schema' then 'SYSTEM TABLE' ELSE "
                    "if(TABLE_TYPE='BASE TABLE','TABLE',TABLE_TYPE) END AS TABLE_TYPE, NULL AS REMARKS FROM ("
                    "SELECT * FROM \"",
                    524288, 512); 
                MADB_DynstrAppend(&StmtStr, Tabptr);
                MADB_DynstrAppend(&StmtStr, "\".INFORMATION_SCHEMA.TABLES ");
                ...
                MADB_DynstrAppend(&StmtStr, ")) WHERE 1=1 ");

                MADB_DynstrAppend(&StmtStr, " AND TABLE_CAT LIKE ");
        ...
        else if (Stmt->Connection->CatalogName)
        {
            MADB_InitDynamicString(&StmtStr, "SELECT * FROM (SELECT TABLE_CATALOG AS TABLE_CAT, TABLE_SCHEMA AS TABLE_SCHEM, TABLE_NAME, "
                "CASE TABLE_SCHEMA WHEN 'information_schema' then 'SYSTEM TABLE' ELSE "
                "if(TABLE_TYPE='BASE TABLE','TABLE',TABLE_TYPE) END AS TABLE_TYPE ,"
                "NULL AS REMARKS FROM \"",
                8192, 512);

            MADB_DynstrAppend(&StmtStr, Stmt->Connection->CatalogName);
            MADB_DynstrAppend(&StmtStr, "\".INFORMATION_SCHEMA.TABLES) WHERE 1=1 ");
        }
        else if (Stmt->Connection->Dsn->Catalog)
        {
            MADB_InitDynamicString(&StmtStr, "SELECT * FROM (SELECT TABLE_CATALOG AS TABLE_CAT, TABLE_SCHEMA AS TABLE_SCHEM, TABLE_NAME, "
                "CASE TABLE_SCHEMA WHEN 'information_schema' then 'SYSTEM TABLE' ELSE "
                "if(TABLE_TYPE='BASE TABLE','TABLE',TABLE_TYPE) END AS TABLE_TYPE ,"
            ...
            MADB_DynstrAppend(&StmtStr, Stmt->Connection->Dsn->Catalog);
            MADB_DynstrAppend(&StmtStr, "\".INFORMATION_SCHEMA.TABLES) WHERE 1=1 ");
        }

        // TABLE TYPE "SYSTEM TABLE" should be dealt by SchemaName
        if (TableType && TableTypeLength && strcmp(TableType, SQL_ALL_TABLE_TYPES) != 0)
        {
            char* TableTypetoUpper = strupr(TableType);
            int TableTypeisSYS = !stricmp(TableTypetoUpper, "SYSTEM TABLE");      
            if (!TableTypeisSYS)
            {
                if (SchemaName && !stricmp(SchemaName, "information_schema"))
                {
                    MADB_DynstrAppend(&StmtStr, " AND 1=0");
                    goto RETEXEC;
                }
                unsigned int i;
                char *myTypes[2] = { "TABLE", "VIEW" };
                MADB_DynstrAppend(&StmtStr, " AND TABLE_TYPE IN (''");
                for (i = 0; i < 2; i++)
                {
                    if (strstr(TableTypetoUpper, myTypes[i]))
                    {
                            MADB_DynstrAppend(&StmtStr, ", '");
            ...
            else
            {
                if (!SchemaName)
                {
                    MADB_DynstrAppend(&StmtStr, " AND TABLE_SCHEM ");
                    MADB_DynstrAppend(&StmtStr, "LIKE ");
                    MADB_DynstrAppend(&StmtStr, Quote);
                    MADB_DynstrAppend(&StmtStr, "information_schema");
                    MADB_DynstrAppend(&StmtStr, Quote);
                    //Set ESCAPE Character
                    MADB_DynstrAppend(&StmtStr, " ESCAPE ");
                    MADB_DynstrAppend(&StmtStr, Quote);
                ...
                {
                    //Fix the case when TableType is SYS and SchemaName is %, this is OK now
                    int SchemaNameisISorPercent = ( !stricmp(SchemaName, "information_schema") 
                        || !stricmp(SchemaName, "%") );
                    if (!SchemaNameisISorPercent)
                    {
                        MADB_DynstrAppend(&StmtStr, " AND 1=0");
                        goto RETEXEC;
                    }
                    else {
                        if (!stricmp(SchemaName, "%"))
                        {
                            MADB_DynstrAppend(&StmtStr, " AND TABLE_SCHEM ");
                            MADB_DynstrAppend(&StmtStr, "LIKE ");
                            MADB_DynstrAppend(&StmtStr, Quote);
        ...
        if (SchemaName && SchemaNameLength)
        {
            if (stricmp(SchemaName, "%"))
            {
                MADB_DynstrAppend(&StmtStr, " AND TABLE_SCHEM ");
        ...
        MADB_DynstrAppend(&StmtStr, " ORDER BY TABLE_CAT, TABLE_SCHEM, TABLE_NAME, TABLE_TYPE");
    }
    
    RETEXEC: MDBUG_C_PRINT(Stmt->Connection, "SQL Statement: %s", StmtStr.str);
                              ...
                              SQLUSMALLINT Unique, SQLUSMALLINT Reserved)
{
  ...
  length = _snprintf_s(StmtStr, sizeof(StmtStr), sizeof(StmtStr) - 1, "SELECT * FROM (SELECT CAST('NULL' AS VARCHAR(2048)) AS TABLE_CAT, "
                             "CAST('NULL' AS VARCHAR(2048)) AS TABLE_SCHEM, CAST('NOT NULL' AS VARCHAR(2048)) AS TABLE_NAME, 0 AS NON_UNIQUE, "
                             "CAST('NULL' AS VARCHAR(2048)) AS INDEX_QUALIFIER, CAST('NULL' AS VARCHAR(2048)) AS INDEX_NAME, 0 AS TYPE, "
                             "0 AS ORDINAL_POSITION, CAST('NULL' AS VARCHAR(2048)) AS COLUMN_NAME, 'A' AS ASC_OR_DESC, 0 AS CARDINALITY, 0 AS PAGES, "
                             "CAST('NULL' AS VARCHAR(2048)) AS FILTER_CONDITION) WHERE 1=0 ",
                             SQL_INDEX_OTHER);
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);


  length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, "ORDER BY NON_UNIQUE, TYPE, INDEX_NAME, ORDINAL_POSITION");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
                           ...
                           char *ColumnName,  SQLSMALLINT NameLength4)
{
  ...
  // no DSN CatalogName and no input return an empty resultset.
  if (!CatalogName && !(Stmt->Connection->Dsn->Catalog))
  {
  ...
  MADB_InitDynamicString(&StmtStr, ONEQODBC_COLUMNS_OUTSIDESORT, 8192, 1024);

  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_CHAROCTLEN))
      goto dynerror;
  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_COLUMNSIZE))
      goto dynerror;
  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_BUFFERLEN))
      goto dynerror;
  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_MAINBUILD))
      goto dynerror;
  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_DATATYPE))
      goto dynerror;
  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_COLUMNTYPE))
      goto dynerror;
  if (MADB_DynstrAppend(&StmtStr, ONEQODBC_COLUMNS_BRACKETLEN))
      goto dynerror;
  ...
  if (!CatalogName && Stmt->Connection->Dsn->Catalog)
  {
      if (MADB_DynstrAppend(&StmtStr, " from \"") ||
          MADB_DynstrAppend(&StmtStr, Stmt->Connection->Dsn->Catalog) ||
          MADB_DynstrAppend(&StmtStr, "\".INFORMATION_SCHEMA.COLUMNS WHERE 1=1 "))
          goto dynerror;
  } else if (CatalogName)
  {
    if (MADB_DynstrAppend(&StmtStr, " from \"") ||
  ...
  if (SchemaName && NameLength2)
      if (MADB_DynstrAppend(&StmtStr, " AND TABLE_SCHEMA LIKE '") ||
                                ...
                                SQLSMALLINT NameLength3, char *ColumnName, SQLSMALLINT NameLength4)
{
  ...
  length = _snprintf_s(p, sizeof(StmtStr), sizeof(StmtStr) - 1, ONEQODBC_PROCEDURE_COLUMNS);
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);

  length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, " ORDER BY PROCEDURE_CAT, PROCEDURE_NAME, ORDINAL_POSITION");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
                                ...
                                SQLSMALLINT NameLength3)
{
  ...
  length = _snprintf_s(p, sizeof(StmtStr), sizeof(StmtStr) - 1, "SELECT * FROM (SELECT CAST('NULL' AS VARCHAR(2048)) AS TABLE_CAT, "
                           "CAST('NULL' AS VARCHAR(2048)) AS TABLE_SCHEM, CAST('NULL' AS VARCHAR(2048)) AS TABLE_NAME, "
                           "CAST('NOT NULL' AS VARCHAR(2048)) AS COLUMN_NAME, CAST(0 AS SMALLINT) AS KEY_SEQ, "
                           "CAST('NULL' AS VARCHAR(2048)) AS PK_NAME) WHERE 1=0");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);

  length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, " ORDER BY TABLE_CAT, TABLE_SCHEM, TABLE_NAME, KEY_SEQ");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
                                  ...
                                  SQLUSMALLINT Scope, SQLUSMALLINT Nullable)
{
  ...
  length = _snprintf_s(p, sizeof(StmtStr), sizeof(StmtStr) - 1, "SELECT * FROM (SELECT CAST(0 AS SMALLINT) AS SCOPE, "
                            "CAST('NOT NULL' AS VARCHAR(2048)) AS COLUMN_NAME, CAST(0 AS SMALLINT) AS DATA_TYPE, "
                            "CAST('NOT NULL' AS VARCHAR(2048)) as TYPE_NAME, 0 as COLUMN_SIZE, "
                            "0 AS BUFFER_LENGTH, CAST(0 AS SMALLINT) as DECIMAL_DIGITS, "
                            "CAST(0 AS SMALLINT) as PSEUDO_COLUMN) where 1=0 ");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);

  length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, "ORDER BY SCOPE");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
                              ...
                              SQLSMALLINT NameLength3)
{
  ...
  length = _snprintf_s(p, sizeof(StmtStr), sizeof(StmtStr) - 1, "select * from (select CAST('NULL' AS VARCHAR(2048)) as PROCEDURE_CAT, "
                               "CAST('NULL' AS VARCHAR(2048)) as PROCEDURE_SCHEM, CAST('NOT NULL' AS VARCHAR(2048)) as PROCEDURE_NAME, "
                               "NULL as NUM_INPUT_PARAMS, NULL as NUM_OUTPUT_PARAMS, NULL as NUM_RESULT_SETS,"
                               "CAST('NULL' AS VARCHAR(2048)) as REMARKS, CAST(0 AS SMALLINT) as PROCEDURE_TYPE) where 1=0");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);


  length = _snprintf_s(p, sizeof(StmtStr) - strlen(StmtStr), sizeof(StmtStr) - strlen(StmtStr) - 1, " ORDER BY PROCEDURE_CAT, PROCEDURE_SCHEM, PROCEDURE_NAME");
  p = (length >= 0)? (p + length) : (&StmtStr[sizeof(StmtStr) - 1]);
                               ...
                               SQLSMALLINT NameLength6)
{
  ...
  MADB_InitDynamicString(&StmtStr,
      "SELECT * FROM (SELECT CAST('NULL' AS VARCHAR(2048)) AS PKTABLE_CAT, CAST('NULL' AS VARCHAR(2048)) AS PKTABLE_SCHEM, "
      "CAST('NOT NULL' AS VARCHAR(2048)) AS PKTABLE_NAME, CAST('NOT NULL' AS VARCHAR(2048)) AS PKCOLUMN_NAME, "
      "CAST('NULL' AS VARCHAR(2048)) AS FKTABLE_CAT, CAST('NULL' AS VARCHAR(2048)) AS FKTABLE_SCHEM, "
      "CAST('NOT NULL' AS VARCHAR(2048)) AS FKTABLE_NAME, CAST('NOT NULL' AS VARCHAR(2048)) AS FKCOLUMN_NAME, "
      "CAST(0 AS SMALLINT) AS KEY_SEQ, CAST(0 AS SMALLINT) AS UPDATE_RULE, "
      "CAST(0 AS SMALLINT) AS DELETE_RULE, CAST('NULL' AS VARCHAR(2048)) AS FK_NAME, "
      "CAST('NULL' AS VARCHAR(2048)) AS PK_NAME, CAST(0 AS SMALLINT) AS DEFERRABILITY) WHERE 1=0 ",
      8192, 128);
